...
interface BotEntry {
    ...
    /**
     * How many times this bot has died since it first joined.
     *
...
export function startApiServer(port: number): void {
    ...
    app.post('/:bot/action', async (req, res) => {
        ...
        try {
            const result = await action.execute(bot.bot, bot.map);
            //wait for 1 tick after executing the action to ensure environment is updated
